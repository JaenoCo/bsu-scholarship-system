...
class SearchController extends Controller
{
    ...
    public function suggest(Request $request)
    {
        ...
        $payload = $this->buildSearchPayload($term, $user, $campusIds);
    ...
    public function results(Request $request)
    {
        ...
        if (mb_strlen($term) >= self::MIN_TERM_LENGTH) {
            $campusIds = $this->resolveCampusScope($user);
            $results = $this->buildSearchPayload($term, $user, $campusIds);
            $groups = array_keys(array_filter($results, fn ($items) => is_array($items) && count($items) > 0));
            $filteredResults = $group === 'all' ? $results : [$group => $results[$group] ?? []];
        }
    ...
    protected function buildSearchPayload(string $term, User $user, ?array $campusIds): array
    {
