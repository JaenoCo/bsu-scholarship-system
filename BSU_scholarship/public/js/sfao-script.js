    ...
    return {
        ...
        init() {
            try {
                ...
                if (Object.keys(this.analyticsData).length === 0) {
                    ...
                    if (dataEl) {
                        try {
                            this.analyticsData = JSON.parse(
                                dataEl.dataset.analytics || "{}",
                            );
                        } catch (e) {
                            console.error(
                                "Error parsing inline analytics data",
                                e,
                            );
                        }
                    }
                ...
                if (
                    savedViewMode &&
                    ["applicants", "scholars"].includes(savedViewMode)
                ) {
                ...
                if (currentTab === "analytics_scholarships") {
                    ...
                    currentTab === "analytics_applications" ||
                    currentTab === "analytics_applicants"
                ) {
                ...
                console.error(
                    "CRITICAL ERROR in SFAO Statistics Tab init:",
                    error,
                );
            }
        },
        ...
        handleTabChange(newTab) {
            ...
            if (newTab === "analytics" || newTab.startsWith("analytics_")) {
                if (newTab === "analytics_scholarships")
                    this.setSubTab("scholarships");
                else if (newTab === "analytics_applications")
        ...
        normalizeCollegeCode(label) {
            ...
            return (
                Object.keys(this.getCollegeColorMap()).find(
        ...
        getActiveApplicantStatusKeys() {
            return this.getApplicantStatusKeys().filter(
                (key) => this.chartLegend[key],
            );
        },
        ...
        updateCollegesList(campusId) {
            if (campusId === "all") {
                ...
                const campusShortNames =
                    (this.analyticsData.campus_colleges || {})[campusId] || [];
                this.availableColleges = (
                    this.analyticsData.all_colleges || []
                ).filter((d) => campusShortNames.includes(d.short_name));
            }

            // Reset college selection if invalid
            if (
                this.localFilters.college !== "all" &&
                !this.availableColleges.find(
                    (d) => d.short_name === this.localFilters.college,
                )
            ) {
                this.localFilters.college = "all";
            }
        ...
        updateProgramList() {
            ...
            if (
                ...
                !this.availablePrograms.includes(this.localFilters.program)
            ) {
                this.localFilters.program = "all";
            }
        },
        ...
        updateTrackList() {
            ...
            if (program === "all") {
                ...
                if (
                    this.localFilters.track !== "all" &&
                    !this.availableTracks.includes(this.localFilters.track)
                ) {
                    this.localFilters.track = "all";
                }
            }
        ...
        updateFilteredData() {
            ...
            let allApplications =
                this.analyticsData.all_applications_data ||
                this.analyticsData.applications ||
                [];

            // 1. Filter College Stats based on Campus (Sidebar)
            let allowedColleges = [];
            if (this.filters.campus === "all") {
                const allCols = Object.values(
                    this.analyticsData.campus_colleges || {},
                ).flat();
                ...
                allStudents = allStudents.filter(
                    (s) => s.campus_id == this.filters.campus,
                );
                // Filter applications by campus
                allApplications = allApplications.filter(
                    (a) => a.campus_id == this.filters.campus,
                );
            }

            if (data.college_stats) {
                data.college_stats = data.college_stats.filter((d) =>
                    allowedColleges.includes(d.name),
                );
            }
            ...
            if (this.localFilters.college !== "all") {
                allApplications = allApplications.filter(
                    (item) => item.college === this.localFilters.college,
                );
            }
            // Filter by Program
            if (this.localFilters.program !== "all") {
                allApplications = allApplications.filter(
                    (item) => item.program === this.localFilters.program,
                );
            }
            // Filter by Track
            if (this.localFilters.track !== "all") {
                allApplications = allApplications.filter(
                    (item) => item.track === this.localFilters.track,
                );
            }
            ...
            const summaryCounts = {
                ...
                applicantsCount: 0,
                scholarsCount: 0,
                applicantsUserIds: new Set(),
                scholarsUserIds: new Set(),
                allUserIds: new Set(),
            };

            const applicantStatusByUser = new Map();
            const scholarTypeByUser = new Map();
            const uniqueScholarIds = new Set();
            const validApplicantStatuses = new Set([
                "pending",
                "approved",
                "rejected",
                "in_progress",
            ...
            const getOrCreateStudentGroup = (item) => {
                let group = studentGroups.find((entry) =>
                    this.areSameStudent(entry.representative, item),
                );
                if (!group) {
                    group = {
                        representative: item,
                        hasScholar: false,
                        hasApplicant: false,
                        applicantStatuses: new Set(),
                        scholarTypes: new Set(),
                    };
            ...
            allApplications.forEach((app) => {
                const identity = this.getStudentIdentity(app);
                const isScholarVal =
                    Number(app.is_global_scholar) || (app.scholar_id ? 1 : 0);
                const isScholar = isScholarVal > 0;
                const hasApplicantStatus = validApplicantStatuses.has(app.status);

                if (!identity) return;

                const group = getOrCreateStudentGroup(app);
                if (isScholar) {
                    group.hasScholar = true;
                    group.scholarTypes.add(this.getScholarRecordType(app));
                }
                if (hasApplicantStatus) {
                    group.hasApplicant = true;
                    group.applicantStatuses.add(app.status);
                }
            });

            studentGroups.forEach((group) => {
                const identity = this.getStudentIdentity(group.representative);
                if (!identity) return;

                if (this.viewMode === "scholars") {
                    if (!group.hasScholar) return;
                    uniqueScholarIds.add(identity);
                    // "New" wins if the student has any new-scholar record;
                    // otherwise they're counted as a continuing/old scholar.
                    scholarTypeByUser.set(
                        identity,
                        group.scholarTypes.has("new") ? "new" : "old",
                    );
                    return;
                }

                if (this.viewMode === "applicants") {
                    if (!group.hasApplicant) return;

                    // A student can simultaneously have multiple applications in
                    // different states (e.g. approved for one scholarship while
                    // still in_progress/pending for another). Resolve to a single
                    // "highest status wins" bucket per student, consistent with the
                    // priority rule used elsewhere: approved > in_progress > pending > rejected.
                    const statuses = group.applicantStatuses;
                    let resolvedStatus = "rejected";
                    if (statuses.has("approved")) {
                        ...
                        resolvedStatus = "active"; // In Progress + Pending are merged for display
                    } else if (statuses.has("rejected")) {
                        resolvedStatus = "rejected";
                    }

                    applicantStatusByUser.set(identity, resolvedStatus);
                    return;
                }

                if (this.viewMode === "comparison") {
                    summaryCounts.allUserIds.add(identity);
                    if (group.hasScholar) summaryCounts.scholarsUserIds.add(identity);
                    if (group.hasApplicant) summaryCounts.applicantsUserIds.add(identity);
                }
            });

            if (this.viewMode === "comparison") {
                summaryCounts.total = summaryCounts.allUserIds.size;
            } else if (this.viewMode === "applicants") {
                summaryCounts.total = applicantStatusByUser.size;
                for (const status of applicantStatusByUser.values()) {
                    if (status === "approved") summaryCounts.approved++;
                    else if (status === "rejected") summaryCounts.rejected++;
                    else if (status === "active") summaryCounts.active++;
                }
                summaryCounts.pending = summaryCounts.active;
            } else if (this.viewMode === "scholars") {
                summaryCounts.total = uniqueScholarIds.size;
                for (const type of scholarTypeByUser.values()) {
                    if (type === "new") summaryCounts.newScholars++;
                    else summaryCounts.oldScholars++;
                }
            }

            // Convert unique user sets into counts for the metric cards
            summaryCounts.applicantsCount = summaryCounts.applicantsUserIds.size;
            summaryCounts.scholarsCount = summaryCounts.scholarsUserIds.size;

            // NOTE: summaryCounts.active/pending were already tallied above from
        ...
        // - applicantsInCampus: unique applicants, respecting all active filters
        //   (campus, search, time period, college, program, track)
        // - approvedApplicants: unique students with at least one approved application (i.e. scholars)
        // - ranking: scholarships sorted by number of unique applicants, highest first
        computeScholarshipDistribution() {
            const all =
                this.analyticsData.all_applications_data ||
                this.analyticsData.applications ||
                [];
            ...
            const isScholarItem = (item) => {
                const val =
                    Number(item.is_global_scholar) ||
                    (item.scholar_id ? 1 : 0);
                return val > 0;
            };

            // Unique students who are applicants (not already a global scholar)
            const countApplicants = (list) => {
                const ids = new Set();
                list.forEach((item) => {
            ...
            // Unique students with at least one approved application (= scholars)
            const countApprovedApplicants = (list) => {
                const ids = new Set();
                list.forEach((item) => {
        ...
        getStatisticsHeader() {
            if (this.filters.campus === "all" || !this.campusOptions.length)
        ...
        getCurrentFilterLabel() {
            ...
            if (this.filters.campus !== "all") {
                ...
                label =
                    "Campus: " + (campus ? campus.name : this.filters.campus);
            }
            if (this.localFilters.college !== "all") {
                label +=
                    (this.filters.campus === "all" ? " | " : ", ") +
                    "College: " +
                    this.localFilters.college;
            }
            return label;
        },
        ...
        async openStudentDetails(scope, type = null) {
            const metricType = type ?? scope;
            const scopeContext = type !== null ? scope : "summary";

            this.isAnonymized = false;
            this.activeView = `${scopeContext}:${metricType}`;
            this.studentDetails = {
                open: true,
                title: this.getStudentDetailsTitle(scopeContext, metricType),
                rows: [],
            };

            const rows = await this.getStudentDetailRows(scopeContext, metricType);
            this.studentDetails = {
                open: true,
                title: this.getStudentDetailsTitle(scopeContext, metricType),
                rows,
        ...
        getStudentIdentityCandidates(item) {
            const candidates = [
                item.user_id,
                item.userId,
                item.student_id,
        ...
        getStudentIdentity(item) {
            const candidates = this.getStudentIdentityCandidates(item);
            const resolved = candidates[0];

            return resolved !== undefined
                ? resolved
                : `${item.scholarship_name || "student"}-${item.status || "status"}`;
        },

        areSameStudent(itemA, itemB) {
            if (!itemA || !itemB) return false;

            const candidatesA = this.getStudentIdentityCandidates(itemA);
            const candidatesB = this.getStudentIdentityCandidates(itemB);

            if (!candidatesA.length || !candidatesB.length) {
                return this.getStudentIdentity(itemA) === this.getStudentIdentity(itemB);
            }

            const matchingValues = candidatesA.filter((value) =>
                candidatesB.includes(value),
            );
        ...
        async getStudentDetailRows(scope, type) {
            ...
            const getStatusPriority = (item) => {
                if (this.viewMode === "applicants") {
                    ...
                    if (["pending", "in_progress"].includes(item.status)) return 3;
                    if (item.status === "rejected") return 2;
                    return 1;
                }

                if (this.isScholarRecord(item)) return 5;
                if (item.status === "approved") return 4;
            ...
            source.forEach((item) => {
                const identity = this.getStudentIdentity(item);
                if (!identity) return;

                const isScholar = this.isScholarRecord(item);
                const hasValidApplicantStatus = validApplicantStatuses.has(item.status);

                let include = false;
                if (scope === "comparison") {
                    if (metricType === "applicants")
                        include = hasValidApplicantStatus;
                    else if (metricType === "scholars") include = isScholar;
                    else
                        include =
                            isScholar ||
                            hasValidApplicantStatus;
                } else {
                    if (this.viewMode === "scholars" && !isScholar) return;
                    if (
                        this.viewMode === "applicants" &&
                    ...
                    if (metricType === "approved" || metricType === "approvalRate") {
                        include = this.viewMode === "applicants" && item.status === "approved";
                    } else if (metricType === "rejected") {
                        include = this.viewMode === "applicants" && item.status === "rejected";
                    } else if (metricType === "active") {
                        include =
                            this.viewMode === "applicants" &&
                            ["pending", "in_progress"].includes(item.status);
                    } else if (metricType === "newScholars") {
                        include =
                            this.viewMode === "scholars" &&
                            this.getScholarRecordType(item) === "new";
                    } else if (metricType === "oldScholars") {
                        include =
                            this.viewMode === "scholars" &&
                            this.getScholarRecordType(item) === "old";
                    } else {
                        include = true;
                    }
                }
            ...
            const rows = studentGroups
                .map((group, index) => {
                    const representative = group.items.reduce((best, current) => {
                        ...
                        const existingPriority = getStatusPriority(best);
                        const currentPriority = getStatusPriority(current);
                        return currentPriority > existingPriority ? current : best;
                    }, null);

                    if (!representative) return null;
                    return this.formatStudentDetailRow(representative, index);
                })
                .filter(Boolean)
            ...
            return Promise.all(
                rows.map(async (row) => ({
                    ...row,
                    name: this.isAnonymized
                        ? await this.anonymizeName(row.name)
                        : row.name,
                })),
            );
        },
        ...
        isScholarRecord(item) {
            return (
                ...
                (Number(item.is_global_scholar) || (item.scholar_id ? 1 : 0)) >
                0
            );
        },

        getScholarRecordType(item) {
            if (!item) return "old";

            if (item.status === "approved") {
                const rawDate = item.updated_at || item.approved_at || item.created_at;
                const approvedDate = rawDate ? new Date(rawDate) : null;
            ...
            return item.scholar_type === "old" ? "old" : "new";
        },
        ...
        formatStudentDetailRow(item, index) {
            const nameParts = [
                ...
                .filter(Boolean);
            const campus = this.campusOptions.find(
                (c) => String(c.id) === String(item.campus_id),
            );

            const identity = this.getStudentIdentity(item);

            return {
                key: `${identity}-${item.scholarship_name || "scholarship"}-${item.status || "status"}-${index}`,
                studentNumber:
                    item.sr_code ||
                    item.student_number ||
                    item.student_no ||
                ...
                campus: campus
                    ? campus.name
                    : item.campus_name || item.campus || "Unassigned campus",
        ...
        async anonymizeName(name) {
            ...
            this.nameHashCache[rawName] = rawName
                .split(/\s+/)
                .map((part) => {
                    if (!part) return "";
        ...
        buildGranularBreakdown(rawData, mode = "comparison") {
            ...
            rawData.forEach((item) => {
                ...
                if (mode === "scholars") {
                    if (!isScholar) return;
                    if (this.getScholarRecordType(item) === "new")
                        statusCounts.new_scholars++;
                ...
                mode === "scholars"
                    ...
                    : [
                          [
                              ...
                              this.getApplicantStatusColor("approved"),
                          ],
                          [
                              "Rejected",
                              statusCounts.rejected,
                              this.getApplicantStatusColor("rejected"),
                          ],
                          [
                              "Pending Review",
                          ...
                          ...(mode === "comparison"
                              ? [["Scholars", statusCounts.scholars, "#7C3AED"]]
                              : []),
                      ];
        ...
        _executeCreateAllCharts(retryCount = 0) {
            if (typeof Chart === "undefined") {
                if (retryCount > 20) {
                    console.warn(
                        "Chart.js failed to load after multiple retries.",
                    );
                    return;
                }
            ...
            if (ctx.clientWidth === 0 || ctx.clientHeight === 0) {
                if (retryCount > 10) {
                    console.warn(
                        `Canvas ${primaryCanvasId} found but remains hidden (0 dimensions). Stopping retries.`,
                    );
                    return;
                }
        ...
        createCollegeChart() {
            ...
            const compactBreakdown = this.buildGranularBreakdown(
                rawData,
                this.viewMode,
            );
            ...
            rawData.forEach((item) => {
                ...
                if (this.viewMode === "applicants") {
                    if (item.status === "pending")
                        groupedData[groupKey].pending.add(item.user_id);
                    else if (item.status === "approved")
                        groupedData[groupKey].approved.add(item.user_id);
                    else if (item.status === "rejected")
                        groupedData[groupKey].rejected.add(item.user_id);
                    else if (item.status === "in_progress")
                        groupedData[groupKey].inProgress.add(item.user_id);
                } else {
                    // Scholars Mode
                    if (this.isScholarRecord(item)) {
                        if (this.getScholarRecordType(item) === "new")
                            groupedData[groupKey].newScholars.add(item.user_id);
                        else
                            groupedData[groupKey].oldScholars.add(item.user_id);
                    }
                }
            });
            ...
            const labels = allLabels.filter((name) => {
                ...
                if (this.viewMode === "applicants") {
                    if (this.chartLegend.approved)
                        visibleCount += data.approved.size;
                    if (this.chartLegend.pending)
                        visibleCount += data.pending.size;
                    if (this.chartLegend.rejected)
                        visibleCount += data.rejected.size;
                    if (this.chartLegend.inProgress)
                        visibleCount += data.inProgress.size;
                } else {
                    if (this.chartLegend.newScholars)
                        visibleCount += data.newScholars.size;
                    if (this.chartLegend.oldScholars)
                        visibleCount += data.oldScholars.size;
                }
                return visibleCount > 0;
            });
            const pendingData = labels.map((l) => groupedData[l].pending.size);
            const approvedData = labels.map(
                (l) => groupedData[l].approved.size,
            );
            const rejectedData = labels.map(
                (l) => groupedData[l].rejected.size,
            );
            const inProgressData = labels.map(
                (l) => groupedData[l].inProgress.size,
            );
            const applicantData = labels.map((l) => {
                let count = 0;
                if (this.chartLegend.pending)
                    count += groupedData[l].pending.size;
                if (this.chartLegend.rejected)
                    count += groupedData[l].rejected.size;
                if (this.chartLegend.inProgress)
                    count += groupedData[l].inProgress.size;
                return count;
            });
            const newScholarsData = labels.map(
                (l) => groupedData[l].newScholars.size,
            );
            const oldScholarsData = labels.map(
                (l) => groupedData[l].oldScholars.size,
            );
            const scholarsData = labels.map((l) => {
                let count = 0;
                if (this.chartLegend.newScholars)
                    count += groupedData[l].newScholars.size;
                if (this.chartLegend.oldScholars)
                    count += groupedData[l].oldScholars.size;
                return count;
            });
            ...
            const processedLabels = labels.map((label) => {
                ...
                words.forEach((word) => {
                    ...
                    if (
                        ...
                        currentLine.join(" ").length + word.length > 15
                    ) {
            ...
            const hasData = labels.some((l) => {
                ...
                if (groupedData[l]) {
                    count +=
                        groupedData[l].pending.size +
                        groupedData[l].approved.size +
                        groupedData[l].rejected.size +
                        groupedData[l].inProgress.size +
                        groupedData[l].newScholars.size +
                        groupedData[l].oldScholars.size;
                }
                return count > 0;
            });
            ...
            this.chartStatus = {
                ...this.chartStatus,
                college: labels.length > 0 && hasData,
            };
            this.chartStatus = {
                ...this.chartStatus,
                college: labels.length > 0 && hasData,
        ...
        createTrendChart() {
            ...
            if (
                ...
                const selectedScholarship = (
                    this.analyticsData.available_scholarships || []
                ).find(
                    (s) => String(s.id) === String(this.filters.scholarship),
                );
            ...
            rawData.forEach((item) => {
                ...
                if (
                    this.viewMode === "scholars" &&
                    (item.status !== "approved" || !item.scholar_id)
                )
                    return;
                ...
                if (this.viewMode === "applicants") {
                    if (item.status === "pending") groupedData[key].pending++;
                        ...
                        groupedData[key].approved++;
                    else if (item.status === "rejected")
                        groupedData[key].rejected++;
                    else if (item.status === "in_progress")
                        groupedData[key].in_progress++;
                } else {
                    ...
                    if (this.isScholarRecord(item)) {
                        if (this.getScholarRecordType(item) === "new") groupedData[key].new++;
        ...
        createComparisonChart() {
            ...
            rawData.forEach((item) => {
                ...
                if (isSearchActive) {
                    ...
                    if (this.filters.campus === "all") {
                        ...
                        if (item.campus_name) {
                            ...
                            const cObj = this.campusOptions.find(
                                (c) => c.id == item.campus_id,
                            );
                            name = cObj
                                ? cObj.name
                ...
                if (
                    ["pending", "approved", "rejected", "in_progress"].includes(
                        item.status,
                    )
                )
                    groupedData[name].applicantsSet.add(item.user_id);

                // Always populate Detailed Status Sets
                if (item.status === "pending")
                    groupedData[name].pending.add(item.user_id);
                else if (item.status === "approved")
                    groupedData[name].approved.add(item.user_id);
                else if (item.status === "rejected")
                    groupedData[name].rejected.add(item.user_id);
                else if (item.status === "in_progress")
                    groupedData[name].inProgress.add(item.user_id);

                // Always populate Scholar Types
                if (isScholar) {
                    if (this.getScholarRecordType(item) === "new")
                        groupedData[name].newScholars.add(item.user_id);
            ...
            const labels = allLabels.filter((name) => {
                ...
                if (isSearchActive) {
                    return (
                        this.chartLegend.applicants || this.chartLegend.scholars
                    );
                }
                ...
                if (this.subTab === "scholarships") {
                    if (this.chartLegend.applicants)
                        visibleCount += data.applicantsSet.size;
                    if (this.chartLegend.scholars)
                        visibleCount += data.scholarsSet.size;
                } else if (this.viewMode === "applicants") {
                    if (this.chartLegend.approved)
                        visibleCount += data.approved.size;
                    if (this.chartLegend.pending)
            ...
            if (isSearchActive) {
                ...
                if (targetName && groupedData[targetName]) {
                    ...
                    datasets = [
                        ...
                        {
                            label: "Approved",
                            data: d(true, stats.approved.size),
                            backgroundColor:
                                this.getApplicantStatusColor("approved"),
                            stack: "main",
                        },
                        {
                            label: "Rejected",
                            data: d(true, stats.rejected.size),
                            backgroundColor:
                                this.getApplicantStatusColor("rejected"),
                            stack: "main",
                        },
                        {
                            label: "Pending",
                            data: d(true, stats.pending.size),
                            backgroundColor:
                                this.getApplicantStatusColor("pending"),
                            stack: "main",
                        },
                        {
                            label: "In Progress",
                            data: d(true, stats.inProgress.size),
                            backgroundColor:
                                this.getApplicantStatusColor("inProgress"),
                            stack: "main",
                        },
                        ...
                        {
                            label: "New Scholars",
                            data: d(false, stats.newScholars.size),
                            backgroundColor: "#EF4444",
                            stack: "main",
                        },
                        {
                            label: "Old Scholars",
                            data: d(false, stats.oldScholars.size),
                ...
                // DEFAULT MODE
                const applicantsData = labels.map(
                    (l) => groupedData[l].applicantsSet.size,
                );
                ...
                datasets = [
                    {
                        label: "Applicants",
                        data: applicantsData,
                        backgroundColor: applicantColors,
                        stack: "total",
                    },
                    {
                        label: "Scholars",
        ...
        createScholarshipRankingChart() {
            const ctx = document.getElementById("sfaoScholarshipRankingChart");
            if (!ctx) return;
            if (chartInstances.scholarshipRanking) {
                chartInstances.scholarshipRanking.destroy();
                chartInstances.scholarshipRanking = null;
            }

            const ranking =
                (this.filteredData.scholarshipDistribution &&
                    this.filteredData.scholarshipDistribution.ranking) ||
                [];
            const top = ranking.slice(0, 5);
            if (top.length === 0) return;

            chartInstances.scholarshipRanking = new Chart(ctx, {
                type: "bar",
                data: {
                ...
                options: {
                    indexAxis: "y", // Horizontal bars, easiest to scan for a ranking
                    responsive: true,
                    maintainAspectRatio: false,
                    scales: {
                        x: {
        ...
        createScholarshipStatusChart() {
            ...
            if (chartInstances.scholarshipType)
                chartInstances.scholarshipType.destroy();

            const rawLabels = Object.keys(
                this.filteredData.scholarshipStats || {},
            );
            const labels = rawLabels.map((name) => name.split(" "));
            const scholars = rawLabels.map(
                (name) => this.filteredData.scholarshipStats[name].scholars,
            );
            const nonScholars = rawLabels.map(
                (name) => this.filteredData.scholarshipStats[name].nonScholars,
            );
            ...
            chartInstances.scholarshipType = new Chart(ctx, {
                ...
                options: {
                    ...
                    scales: {
                        ...
                        y: { ticks: { color: this.getTextColor() } },
                    },
                },
            });
        },
        ...
        handleSearchInput() {
            ...
            if (this.analyticsData.campus_scholarships) {
                ...
                if (currentCampus === "all") {
                    list = Object.values(
                        this.analyticsData.campus_scholarships,
                    ).flat();
                } else {
                    list =
                        this.analyticsData.campus_scholarships[currentCampus] ||
                        [];
                }
                list.forEach((item) =>
                    sourceNames.add(item.name || item.scholarship_name),
                );
            }
            // 2. Try 'all_scholarships' (Canonical list)
            else if (
                this.analyticsData.all_scholarships &&
                this.analyticsData.all_scholarships.length > 0
            ) {
                this.analyticsData.all_scholarships.forEach((s) => {
                    // Check campus association (assuming s.campus_id exists)
                    if (
                        currentCampus === "all" ||
                        String(s.campus_id) === String(currentCampus)
                    ) {
            ...
            // 3. Fallback: 'all_applications_data' (History - may include extra items)
                ...
                history.forEach((d) => {
                    if (
                        currentCampus === "all" ||
                        String(d.campus_id) === String(currentCampus) ||
                        d.campus_name === currentCampus
                    ) {
                        if (d.scholarship_name)
                            sourceNames.add(d.scholarship_name);
                    }
                });
            }
...
window.sfaoDashboardState = function (config) {
    return {
        ...
        urlMapping: {
            ...
            "analytics-scholarships": "analytics_scholarships",
            ...
            applicants: "applicants",
            "applicants-not_applied": "applicants-not_applied",
            "applicants-in_progress": "applicants-in_progress",
            all_applicants: "applicants",
            applicants_not_applied: "applicants-not_applied",
            applicants_in_progress: "applicants-in_progress",
            "applicants-in-progress": "applicants-in_progress",
            ...
            login_security: "account-security",
            "login-and-security": "account-security",
            login_and_security: "account-security",
        },
        ...
        canonicalUrlKeys: {
            ...
            "applicants-not_applied": "applicants_not_applied",
            "applicants-in_progress": "applicants_in_progress",
            "applicants-pending": "applicants_pending",
            "applicants-approved": "applicants_approved",
            "applicants-rejected": "applicants_rejected",
            scholars: "all_scholars",
            "scholars-new": "new_scholars",
            "scholars-old": "old_scholars",
            "reports-applicant_summary": "reports_applicant_summary",
            "reports-scholar_summary": "reports_scholar_summary",
            "reports-grant_summary": "reports_grant_summary",
            "all-app-forms": "all-app-forms",
            "up-app-form": "up-app-form",
            "import-scholarships": "import-scholarships",
            "account-info": "account-info",
        ...
        normalizeTab(rawTab) {
            ...
            const raw = String(rawTab).trim();
            if (!raw) return null;

            if (this.urlMapping[raw]) return this.urlMapping[raw];

            const lower = raw.toLowerCase();
            if (this.urlMapping[lower]) return this.urlMapping[lower];

            const hyphenated = lower.replace(/_/g, "-");
            if (this.urlMapping[hyphenated]) return this.urlMapping[hyphenated];

            const underscored = lower.replace(/-/g, "_");
            if (this.urlMapping[underscored]) return this.urlMapping[underscored];
        ...
        init() {
            ...
            // Restore Dropdowns
            const savedDropdowns = localStorage.getItem(
                `sfaoDropdowns_${config.userId}`,
            );
            if (savedDropdowns) {
                try {
                    this.openDropdowns = JSON.parse(savedDropdowns);
                } catch (e) {
                    console.error("Error parsing dropdown state", e);
                }
            }
            ...
            const normalizedUrlTab = this.normalizeTab(urlTab);

            if (urlTab && normalizedUrlTab) {
                this.tab = normalizedUrlTab;
            ...
            this.updateUrl(this.tab);
            this.$nextTick(() => this.broadcastTabChange(this.tab));
        },

        switchTab(nextTab) {
            nextTab = this.normalizeTab(nextTab) || nextTab;
            ...
            if (previousTab === nextTab) {
                ...
                this.broadcastTabChange(nextTab);
            }
        },
        ...
        updateUrl(currentTab) {
            ...
            if (currentTab === "analytics") {
                ...
                key = this.canonicalUrlKeys[currentTab] || null;
            }

            if (!key) {
                key = Object.keys(this.urlMapping).find(
                    (k) => this.urlMapping[k] === currentTab,
                );
            }
        ...
        syncDropdowns(currentTab) {
            if (
                ...
                this.openDropdowns.dashboard = true;
            else if (currentTab.startsWith("scholarships"))
                this.openDropdowns.scholarships = true;
            else if (currentTab.startsWith("applicants"))
                this.openDropdowns.applicants = true;
            else if (currentTab.startsWith("scholars"))
                this.openDropdowns.scholars = true;
            else if (currentTab.startsWith("reports"))
                this.openDropdowns.reports = true;
            else if (
                currentTab === "all-app-forms" ||
                currentTab === "up-app-form"
            )
                this.openDropdowns.applicationForms = true;
            else if (currentTab.startsWith("account"))
...
window.sfaoApplicantsFilter = function (config) {
    const normalizeApplicantStatus = (status) =>
        String(status || "all").replace("in-progress", "in_progress");
    const normalizeApplicantTab = (tab) =>
        String(tab || "applicants")
            .toLowerCase()
            .replace("applicants-in-progress", "applicants-in_progress");
    const initialTab = normalizeApplicantTab(config.activeTab || "applicants");
    const initialStatus = initialTab === "applicants"
        ? "all"
        : initialTab.startsWith("applicants-")
            ? normalizeApplicantStatus(initialTab.replace("applicants-", ""))
            : null;
    ...
    return {
        ...
        filters: {
            ...
            academic_year:
                localStorage.getItem("sfaoApplicantsAcademicYear") || "all",
            scholarship:
                localStorage.getItem("sfaoApplicantsScholarship") || "all",
            status: initialStatus || localStorage.getItem("sfaoApplicantsStatus") || "all",
        },
        ...
        init() {
            ...
            this.$watch("filters.status", (value) => {
                const normalizedStatus = this.normalizeApplicantStatus(value);
                if (value !== normalizedStatus) {
                    this.filters.status = normalizedStatus;
                    return;
                }

                if (this.syncingStatusFromTab) {
                    this.syncingStatusFromTab = false;
                    return;
                }
        ...
        updateColleges(reset = true) {
            ...
            if (this.filters.campus === "all") {
                ...
                let allCols = new Set();
                Object.values(this.campusCollegePrograms).forEach(
                    (campusCols) => {
                        Object.keys(campusCols).forEach((c) => allCols.add(c));
                    },
                );
                this.colleges = Array.from(allCols)
                    .sort()
                    .map((c) => ({ name: c, value: c }));
            } else {
                const campusCols =
                    this.campusCollegePrograms[this.filters.campus] || {};
                this.colleges = Object.keys(campusCols)
                    .sort()
                    .map((c) => ({ name: c, value: c }));
            }
                ...
                campuses.forEach((cols) => {
                    Object.values(cols).forEach((progs) => {
                        if (Array.isArray(progs))
                            progs.forEach((p) => availablePrograms.add(p));
                    });
                });
            } else {
        ...
        openStatusModal(status) {
            this.handleTabChange(
                status === "all" ? "applicants" : `applicants-${status}`,
            );
        ...
        fetchApplicants(page = 1) {
            ...
            if (!this.currentTab || !this.currentTab.startsWith("applicants"))
                return;
        ...
        resetFilters() {
            ...
            localStorage.setItem("sfaoApplicantsScholarship", "all");
            this.fetchApplicants();
        },
        ...
        normalizeApplicantTab(tab) {
            return normalizeApplicantTab(tab);
        },

        handleTabChange(tab) {
            const normalizedTab = this.normalizeApplicantTab(tab);
            this.currentTab = normalizedTab;
            const nextStatus = normalizedTab === 'applicants'
                ? 'all'
                : normalizedTab.startsWith('applicants-')
                    ? this.normalizeApplicantStatus(normalizedTab.replace('applicants-', ''))
                    : this.filters.status;

            if (this.filters.status !== nextStatus) {
                this.syncingStatusFromTab = true;
                this.filters.status = nextStatus;
            }

            localStorage.setItem("sfaoApplicantsStatus", nextStatus);
            this.updateQueryParams();
            this.fetchApplicants();
        },
    };
...
window.sfaoScholarsFilter = function (config) {
    return {
        filters: {
            ...
            academic_year:
                localStorage.getItem("sfaoScholarsAcademicYear") || "all",
            scholarship:
                localStorage.getItem("sfaoScholarsScholarship") || "all",
        ...
        resetFilters() {
            ...
            this.filters.scholarship = "all";
            //this.filters.type = 'all';
            // Always sync on load, not just when type differs from 'all'
            this.fetchScholars();
        },
        ...
        toggleSelectAll() {
            if (this.selectAll) {
                ...
                const checkboxes = document.querySelectorAll(
                    'input[type="checkbox"][\\@change*="toggleScholar"]',
                );
                this.selectedScholars = Array.from(checkboxes)
                    .map((cb) => {
                        const match = cb
                            .getAttribute("@change")
                            .match(/toggleScholar\((\d+)\)/);
        ...
        toggleScholar(scholarId) {
            ...
            this.selectAll =
                this.selectedScholars.length === totalCheckboxes &&
                totalCheckboxes > 0;
        },
        ...
        async bulkMarkClaimed() {
            ...
            if (
                !confirm(
                    `Are you sure you want to mark ${this.selectedScholars.length} scholar(s) as claimed ? This will update their grant history.`,
                )
            ) {
                return;
            }
            ...
            try {
                const response = await fetch(
                    config.routeUrl.replace(
                        "/sfao",
                        "/sfao/scholars/bulk-mark-claimed",
                    ),
                    {
                        method: "POST",
                        headers: {
                ...
                if (data.success) {
                    alert(
                        `Successfully marked ${data.success_count} scholar(s) as claimed.${data.skipped_count > 0 ? ` ${data.skipped_count} scholar(s) were skipped (already claimed).` : ""} `,
                    );
                    this.selectedScholars = [];
        ...
        async markScholarAs(action) {
            ...
            const actionText =
                action === "claimed" ? "claimed" : "disqualified";
            if (
                !confirm(
                    `Are you sure you want to mark this scholar as ${actionText}?`,
                )
            ) {
                return;
            }
            ...
            try {
                const url =
                    action === "claimed"
                        ...
                        : config.routeUrl.replace(
                              "/sfao",
                              `/sfao/scholars/${this.selectedScholarId}/mark-disqualified`,
                          );
                ...
                if (data.success || response.ok) {
                    alert(
                        data.message ||
                            `Scholar marked as ${actionText} successfully.`,
                    );
                    ...
                    alert(
                        data.message ||
                            `Failed to mark scholar as ${actionText}.`,
                    );
                }
            } catch (error) {
                console.error("Error:", error);
                alert(
                    `An error occurred while marking scholar as ${actionText}.`,
                );
            }
        },
        ...
        getHeaderDescription() {
            ...
            if (this.filters.type === "all") {
                ...
                desc = `Scholars approved this year from ${campusName}`;
            } else if (this.filters.type === "old") {
                desc = `Scholars approved before this year from ${campusName}`;
            }
    ...
    return {
        ...
        updateAvailablePrograms(reset = true) {
            ...
            const collectPrograms = (cId) => {
                ...
                if (this.college === "all") {
                    Object.values(campusData).forEach((progs) =>
                        programs.push(...progs),
                    );
            ...
            if (this.campusId === "all") {
                Object.keys(this.campusCollegePrograms).forEach((cId) =>
                    collectPrograms(cId),
                );
            } else {
            ...
            try {
                ...
                fetch(url, {
                    ...
                    .catch((err) => {
                        ...
                        if (container)
                            container.innerHTML =
                                '<p class="text-red-500 text-center py-4">Error loading report data. Please try again.</p>';
                    })
                    .finally(() => {
