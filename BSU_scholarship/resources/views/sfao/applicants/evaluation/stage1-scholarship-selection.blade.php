...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 md:p-6 mb-8">
            <div class="flex flex-col md:flex-row items-center md:items-start md:space-x-4 space-y-4 md:space-y-0">
                <div class="h-16 w-16 rounded-full bg-bsu-red flex items-center justify-center flex-shrink-0">
                    <span class="text-xl font-bold text-white">
                        {{ strtoupper(substr($student->name, 0, 2)) }}
                    </span>
                </div>
                <div class="text-center md:text-left">
                    <h2 class="text-lg md:text-xl font-semibold text-gray-900 dark:text-white">{{ $student->name }}</h2>
                    <p class="text-gray-600 dark:text-gray-400 text-sm md:text-base">{{ $student->email }}</p>
                    <p class="text-xs md:text-sm text-gray-500 dark:text-gray-500">{{ $student->campus->name }}</p>
                </div>
            </div>
        </div>
        ...
        <!-- Progress Indicator -->
        <div class="mb-8">
            <div class="flex items-center justify-between space-x-1 md:space-x-2 overflow-x-auto pb-2">
                <div class="flex items-center flex-shrink-0">
                    <div class="w-8 h-8 bg-bsu-red text-white rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0">1</div>
                    <span class="ml-2 text-xs md:text-sm font-medium text-gray-900 dark:text-white whitespace-nowrap">Select</span>
                </div>
                <div class="w-4 md:w-12 h-1 bg-gray-300 dark:bg-gray-600 flex-shrink-0"></div>
                <div class="flex items-center flex-shrink-0">
                    <div class="w-8 h-8 bg-gray-300 dark:bg-gray-600 text-gray-600 dark:text-gray-400 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0">2</div>
                    <span class="ml-2 text-xs md:text-sm font-medium text-gray-500 dark:text-gray-400 whitespace-nowrap">SFAO</span>
                </div>
                <div class="w-4 md:w-12 h-1 bg-gray-300 dark:bg-gray-600 flex-shrink-0"></div>
                <div class="flex items-center flex-shrink-0">
                    <div class="w-8 h-8 bg-gray-300 dark:bg-gray-600 text-gray-600 dark:text-gray-400 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0">3</div>
                    <span class="ml-2 text-xs md:text-sm font-medium text-gray-500 dark:text-gray-400 whitespace-nowrap">Docs</span>
                </div>
                <div class="w-4 md:w-12 h-1 bg-gray-300 dark:bg-gray-600 flex-shrink-0"></div>
                <div class="flex items-center flex-shrink-0">
                    <div class="w-8 h-8 bg-gray-300 dark:bg-gray-600 text-gray-600 dark:text-gray-400 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0">4</div>
                    <span class="ml-2 text-xs md:text-sm font-medium text-gray-500 dark:text-gray-400 whitespace-nowrap">Review</span>
                </div>
            </div>
        </div>
        ...
        @if($applications->count() > 0)
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
                <div class="px-4 md:px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                    <h3 class="text-base md:text-lg font-medium text-gray-900 dark:text-white">Applied Scholarships</h3>
                    <p class="mt-1 text-xs md:text-sm text-gray-600 dark:text-gray-400">Select a scholarship to evaluate the student's submitted documents.</p>
                </div>
