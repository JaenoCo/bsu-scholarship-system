...
<div x-cloak x-show="showDetails_{{ $scholarship->id }}" x-transition:enter="transition ease-out duration-300"
    ...
    <div class="relative w-full max-w-2xl bg-white dark:bg-gray-800 rounded-2xl shadow-2xl overflow-hidden transform"
        ...
        <div class="bg-bsu-red p-6 text-white">
            ...
            <div class="flex items-center gap-2 mt-2 text-sm opacity-90">
                <span
                    class="bg-white/20 px-2 py-0.5 rounded uppercase tracking-wider">{{ $scholarship->scholarship_type }}</span>
                <span>{{ $scholarship->is_active ? 'Active' : 'Inactive' }}</span>
            </div>
            <button @click="showDetails_{{ $scholarship->id }} = false"
        ...
        <div class="p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <!-- Available Slots -->
                <div
                    class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl border border-blue-100 dark:border-blue-800 text-center">
                ...
                <!-- Applicants -->
                <div
                    class="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-xl border border-yellow-100 dark:border-yellow-800 text-center">
                ...
                <!-- Approved Scholars -->
                <div
                    class="bg-green-50 dark:bg-green-900/20 p-4 rounded-xl border border-green-100 dark:border-green-800 text-center">
                ...
                <div class="mt-6">
                    ...
                    <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-4 flex overflow-hidden">
                        ...
                        @if($approvedPercent > 0)
                            <div class="bg-green-500 h-full" style="width: {{ $approvedPercent }}%"
                                title="{{ $approvedCount }} Approved"></div>
                        @endif

                        <!-- Pending Segment -->
                        @if($pendingPercent > 0)
                            <div class="bg-gray-400 dark:bg-gray-500 h-full" style="width: {{ $pendingPercent }}%"
                                title="{{ $pendingCount }} Applicants Pending Approval"></div>
                        @endif
                    </div>
            ...
            <div class="mt-8 flex justify-end gap-3">
                @if($scholarship->is_active)
                    <form action="{{ route('sfao.scholarships.release-grant', $scholarship->id) }}" method="POST">
                        @csrf
                        <button type="submit"
                            onclick="return confirm('Release grants for {{ $scholarship->scholarship_name }}? Emails will be sent to all active scholars.');"
                            class="px-4 py-2 bg-bsu-red text-white rounded-lg hover:bg-red-700 transition flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z M9 7v10 M9 7h3.5a2.5 2.5 0 010 5H9 M6 9.5h9 M6 12h9">
                                </path>
                            </svg>
                            Release Grant
                        </button>
