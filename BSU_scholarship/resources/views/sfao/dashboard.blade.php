...
<html lang="en" :class="{ 'dark': darkMode }" x-data="{
  ...
  }" x-on:switch-tab.window="tab = $event.detail" @resize.window="isDesktop = window.innerWidth >= 768" x-init="
...
<body class="bg-gray-100 dark:bg-gray-900 dark:text-white min-h-screen font-sans">
  ...
  <!-- Sidebar -->
  <aside
    class="fixed inset-y-0 left-0 w-64 bg-bsu-red text-white dark:bg-gray-800 transform transition-transform duration-300 z-50 flex flex-col"
    ...
    <nav class="mt-6 px-4 pb-4 overflow-y-auto flex-1 space-y-4 custom-scrollbar">
      ...
      <div class="space-y-1">
        ...
        <a href="{{ route('sfao.reports.student-summary', ['student_type' => 'applicants', 'campus_id' => 'all']) }}"
          class="w-full text-left px-4 py-2 rounded hover:bg-bsu-redDark dark:hover:bg-gray-700 transition text-sm flex items-center gap-2 bg-white text-bsu-red dark:bg-gray-200">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <a href="{{ route('sfao.reports.scholar-summary', ['campus_id' => 'all']) }}"
          class="w-full text-left px-4 py-2 rounded hover:bg-bsu-redDark dark:hover:bg-gray-700 transition text-sm flex items-center gap-2 text-white dark:text-white">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <a href="{{ route('sfao.reports.grant-summary', ['campus_id' => 4]) }}"
          class="w-full text-left px-4 py-2 rounded hover:bg-bsu-redDark dark:hover:bg-gray-700 transition text-sm flex items-center gap-2 text-white dark:text-white">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="space-y-1">
          ...
          <button @click="tab = 'all-app-forms'"
            class="w-full text-left px-4 py-2 rounded hover:bg-bsu-redDark dark:hover:bg-gray-700 transition text-sm flex items-center gap-2"
            :class="tab === 'all-app-forms' ? 'sidebar-tab-active' : 'text-white dark:text-white'">
            Application Forms
          </button>

          <button @click="tab = 'up-app-form'"
            class="w-full text-left px-4 py-2 rounded hover:bg-bsu-redDark dark:hover:bg-gray-700 transition text-sm flex items-center gap-2"
            :class="tab === 'up-app-form' ? 'sidebar-tab-active' : 'text-white dark:text-white'">
      ...
      <div class="space-y-1">
        ...
        @foreach($sfaoCampus->getAllCampusesUnder() as $campus)
          <button
            @click="tab = 'statistics'; statsCampus = '{{ $campus->id }}'; $dispatch('set-stats-filter', '{{ $campus->id }}')"
            class="w-full text-left px-4 py-2 rounded hover:bg-bsu-redDark dark:hover:bg-gray-700 transition text-sm flex items-center gap-2"
  ...
  <div x-show="sidebarOpen" x-transition:enter="transition-opacity ease-linear duration-300"
    ...
    x-transition:leave-end="opacity-0" class="fixed inset-0 bg-gray-900/50 z-40 md:hidden" @click="sidebarOpen = false"
    x-cloak>
  </div>
  ...
  <!-- Main Header -->
  <header
    class="flex items-center justify-between px-8 py-4 bg-[#2f2f2f] dark:bg-gray-800 shadow-sm sticky top-0 z-30 border-b border-gray-700 transition-all duration-300"
    ...
    <div class="flex items-center gap-2">
      <button @click="darkMode = !darkMode"
        ...
        <svg x-show="darkMode" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-yellow-400" fill="none"
          viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
        </svg>
        <!-- Moon Icon (for Light Mode) -->
        <svg x-show="!darkMode" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-gray-400" fill="none"
          viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
  ...
  <div class="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-8 py-3 sticky top-16 z-20"
    :class="{ 'md:ml-64': sidebarOpen, 'md:mr-64': rightSidebarOpen }"
    :style="isDesktop && rightSidebarOpen ? 'margin-right: 16rem;' : ''">
    <h2 class="text-sm font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide" x-text="pageTitle()">
    </h2>
  </div>

  <!-- Right Sidebar -->
  <aside
    class="fixed inset-y-0 right-0 w-64 bg-white dark:bg-gray-800 shadow-xl transform transition-transform duration-300 z-50 flex flex-col"
    :class="rightSidebarOpen ? 'translate-x-0' : 'translate-x-full'" x-cloak>
    <div class="h-full flex flex-col py-6 overflow-y-scroll">
        ...
        <div class="flex items-start justify-between">
          ...
          <div class="ml-3 h-7 flex items-center">
            <button @click="rightSidebarOpen = false"
              class="bg-white dark:bg-gray-800 rounded-md text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-bsu-red">
      ...
      <div class="mt-6 relative flex-1 px-4 sm:px-6">
        ...
        <div class="mt-8 border-t border-gray-200 dark:border-gray-700 pt-6 space-y-4">
          <button @click="tab = 'account-info'"
            class="w-full flex items-center px-4 py-3 text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-3 text-gray-500" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <button @click="showLogoutModal = true; rightSidebarOpen = false"
            class="w-full flex items-center px-4 py-3 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition">
  ...
  <main class="p-4 md:p-8 min-h-screen bg-white dark:bg-gray-900 transition-all duration-300"
    ...
    @if (session('success'))
      <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)" x-transition.opacity
        ...
        <div
          class="bg-white dark:bg-gray-800 border border-green-400 text-green-700 dark:text-green-300 px-6 py-5 rounded-xl shadow-xl flex items-center space-x-4">
          <svg class="w-10 h-10 text-green-500 animate-bounce" fill="none" stroke="currentColor" stroke-width="2"
            viewBox="0 0 24 24">
    ...
    @if ($errors->any())
      <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)" x-transition.opacity
        class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-40">
        <div
          class="bg-white dark:bg-gray-800 border border-red-400 text-red-700 dark:text-red-300 px-6 py-5 rounded-xl shadow-xl flex items-center space-x-4">
          <svg class="w-10 h-10 text-red-500 animate-pulse" fill="none" stroke="currentColor" stroke-width="2"
            viewBox="0 0 24 24">
