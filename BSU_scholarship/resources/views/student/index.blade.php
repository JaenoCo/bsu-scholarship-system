...
@section('navbar')
    <x-layout.navbar title="Batangas State University" subtitle="The National Engineering University" :user="$user"
        :profile="false" :settings="true" settings-click="$dispatch('switch-tab', 'account_settings')" :logout="true" />
@endsection
...
@section('content')
    <div x-data="{
      ...
      }" x-init="init()" @notification-changed.window="
      ...
      ">
        ...
        <div x-show="tab === 'applied-scholarships' || tab === 'applied_scholarships' || tab === 'application_tracking'"
            x-transition>
        ...
        <div x-data="{ showWarning: false }" x-on:show-warning.window="showWarning = true" x-show="showWarning" x-cloak
            ...
            <div class="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-2xl overflow-hidden transform transition-all"
                ...
                <div
                    class="bg-gray-50 dark:bg-gray-700/50 px-6 py-4 flex justify-center border-t border-gray-100 dark:border-gray-700">
    ...
    @if(session('success'))
        <div x-data="{ showModal: true }" x-show="showModal" x-cloak
            ...
            <div x-show="showModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
                ...
                x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm" @click="showModal = false"></div>
    ...
    <div x-data="{ showModal: false, scholarshipId: null }" x-init="
         ...
         " x-on:open-withdraw-modal.window="
            ...
            showModal = true;
         " x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div x-show="showModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95"
            ...
            <div class="flex justify-center mb-6">
                <div class="flex items-center justify-center h-20 w-20 rounded-full bg-yellow-100 dark:bg-yellow-900/30">
                    <svg class="h-10 w-10 text-yellow-600 dark:text-yellow-400" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
