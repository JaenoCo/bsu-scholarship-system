<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        ...
        <div class="flex flex-col md:flex-row gap-8 items-start">
            ...
            <div class="w-full md:w-1/3 flex flex-col items-center space-y-4">
                ...
                <form action="{{ url('/upload-profile-picture/student') }}" method="POST" enctype="multipart/form-data" id="profile_form">
                    @csrf
            ...
            <div class="w-full md:w-2/3 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Full Name</label>
                        <div class="text-lg font-semibold text-gray-800 dark:text-white bg-gray-50 dark:bg-gray-700 px-4 py-2 rounded-md border border-gray-200 dark:border-gray-600">
                            {{ $user->name }}
                        </div>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">SR Code</label>
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-2 bg-gray-50 dark:bg-gray-700 px-4 py-2 rounded-md border border-gray-200 dark:border-gray-600">
                        ...
                        @if($user->email_verified_at)
                            <span class="ml-auto text-xs bg-green-100 text-green-800 px-2 py-0.5 rounded-full font-medium">Verified</span>
                        @else
                            <span class="ml-auto text-xs bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded-full font-medium">Unverified</span>
                        @endif
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Campus</label>
                        <div class="bg-gray-50 dark:bg-gray-700 px-4 py-2 rounded-md border border-gray-200 dark:border-gray-600 text-gray-800 dark:text-white">
                            {{ $user->campus->name ?? 'None' }}
                        </div>
                    </div>
                     <div>
                        <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">College</label>
                        <div class="bg-gray-50 dark:bg-gray-700 px-4 py-2 rounded-md border border-gray-200 dark:border-gray-600 text-gray-800 dark:text-white">
                            {{ $user->college ?? 'N/A' }}
                        </div>
                    </div>
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        ...
        <form action="{{ route('student.change-password') }}" method="POST" class="max-w-xl">
            @csrf
